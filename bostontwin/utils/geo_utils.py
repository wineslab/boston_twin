...
def gdf2crs(
    in_gdf: gpd.GeoDataFrame, crsTransformer: pyproj.Transformer
) -> gpd.GeoDataFrame:
    transformer = partial(transform, crsTransformer.transform)
    out_gdf = in_gdf.set_geometry(in_gdf.geometry.apply(transformer), inplace=False)
...
def get_crs(scene_name: str, scene_center_lon_lat: Tuple):
    ...
    compcrs = CompoundCRS(name=scene_name, components=[projcrs, vertcrs])
    return compcrs
...
def check_area_of_use(
    in_crs: pyproj.CRS, out_crs: pyproj.CRS, in_coords: Union[Tuple, List]
) -> bool:
    # get out_crs bounds
    if len(in_coords) > 2:
        for point_coord in in_coords:
            ...
            return check_point_in_area_of_use(
                out_crs, in_location_out_crs
            ) & check_point_in_area_of_use(in_crs, point_coord)
    else:
        in_location_out_crs = pyproj.Transformer.from_proj(
            in_crs, out_crs, always_xy=True
        ).transform(*in_coords)
