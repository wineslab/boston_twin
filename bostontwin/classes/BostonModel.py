...
class BostonModel:
    ...
    def generate_scene_from_model_gdf(self, model_gdf, scene_center, scene_name) -> None:
        ...
        print(f"Scene size is {scene_size_x:.2f}x{scene_size_y:.2f} m.")
        
        n_models_scene = model_gdf_local.shape[0]
        
        model_list = model_gdf_local["Model_ID"].tolist()
        models_materials = model_gdf_local["StructType"].apply(lambda x: "mat-itu_brick" if "Wall" in x else "mat-itu_concrete").tolist()
        models_centers = model_gdf_local.centroid.apply(lambda x:(x.x, x.y)).tolist()
        scene_center_local = scene_transformer.transform(scene_center["center_lon"], scene_center["center_lat"])
        print(f"Scene center is {scene_center_local}")
