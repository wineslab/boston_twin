...
from src.utils.obj_utils import (
    create_ground_dict,
    dir_obj2ply,
...
from src.utils.mi_utils import create_mitsuba_xml, get_transformation_matrix
...
class BostonModelDownloader:
    def __init__(
        ...
        self.flat = flat
        if self.flat:
            self.out_dataset_dir = out_dataset_dir.with_name(
                out_dataset_dir.name + "_flat"
            )
    ...
    def generate_dataset(self) -> None:
        ...
        global_terrain_model_path = model_dir.joinpath("boston_terrain.ply")
        boston_terrains_path = [model_dir.joinpath(Path(boston_terrains[i]["filename"]).stem) for i in range(0, len(boston_terrains), 2)]
    ...
    def set_local_projections(self):
        ...
        if not original_prj_path.is_file():
            logging.info(
                "Original file not found. Please download it with download_data()."
            )
    ...
    def add_ground_model(
        ...
        if flat:  # if the flat option is enabled, we create a rectangle model as ground
            ...
            ply_path = model_dir.joinpath(terrain_name + ".ply")
