...
def generate_mi_xml(
    ...
    print(f"Generating dict for {scene_name}. create_ground: {create_ground}")
    if create_ground:
        print("Creating ground...")
        # load frame obj and use it as (flat) ground
        frame_name = f"{scene_name}_terrain_flat"
        base_rect_path = models_dir.parents[2].joinpath("data","rectangle.ply")
...
def generate_material_dict(models_materials):
    ...
    for i in range(n_materials):
        material_name_base = material_names[i].split("-")[-1]
        color = [i / n_materials, 1 - i / n_materials, 0.5]
        material_dict[material_name_base] = MATERIAL_BASEDICT
        material_dict[material_name_base].update({'string': {
        'name': 'type',
        'value': 'brick'
    },
    'float': {
